...
class CatlassParameter:
    ...
    def generate_mn_tile_values(self):
        ...
        # 极小值区域（1-8）：步长1，最密集采样
        # 这个区域对性能影响最大，且约束下最常用
        values.extend(range(1, 9, 1))  # 1,2,3,4,5,6,7,8 (8个值)
        
        # 小值区域（10-20）：步长2，密集采样
        # 包含关键值10, 12, 14, 16, 18, 20，这些值组合空间大
        values.extend(range(10, 21, 2))  # 10,12,14,16,18,20 (6个值)
        
        # 中小值区域（22-32）：步长2，中等密度
        # 包含关键值22, 24, 26, 28, 30, 32，仍有一定组合空间
        values.extend(range(22, 33, 2))  # 22,24,26,28,30,32 (6个值)
        
        # 中值区域（36-48）：步长4，稀疏采样
        # 包含关键值36, 40, 44, 48，但组合空间受限
        values.extend(range(36, 49, 4))  # 36,40,44,48 (4个值)
        
        # 大值区域（52-64）：步长4，稀疏采样
        # 包含关键值52, 56, 60, 64，组合空间很小
    ...
    def generate_k_tile_values(self):
        ...
        # 极小值区域（1-8）：步长1，最密集采样
        # kTile对L1缓存和L0A/L0B影响大，极小值需要完整覆盖
        values.extend(range(1, 9, 1))  # 1,2,3,4,5,6,7,8 (8个值)
        
        # 小值区域（10-20）：步长2，密集采样
        # 包含关键值10, 12, 14, 16, 18, 20，这些值在大多数组合中可用
        values.extend(range(10, 21, 2))  # 10,12,14,16,18,20 (6个值)
        
        # 中小值区域（22-32）：步长2，中等密度
        # 包含关键值22, 24, 26, 28, 30, 32
        values.extend(range(22, 33, 2))  # 22,24,26,28,30,32 (6个值)
